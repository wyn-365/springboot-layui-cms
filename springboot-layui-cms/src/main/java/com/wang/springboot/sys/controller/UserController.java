...
import com.wang.springboot.sys.common.*;
...
import com.wang.springboot.sys.vo.UserVo;
...
public class UserController {
    ...
    public ResultObj updateUserInfo(UserVo userVo, HttpServletRequest request){
        try {
            //用户头像不是默认图片
            if (!(userVo.getImgpath()!=null&&userVo.getImgpath().equals(Constast.DEFAULT_IMG_GOODS))){
                if (userVo.getImgpath().endsWith("_temp")){
                    String newName = AppFileUtils.renameFile(userVo.getImgpath(),request);
